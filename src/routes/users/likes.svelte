...
<script>import Icon from '$lib/Icon.svelte';
...
import { onMount } from 'svelte';
  ...
  async function handleUnlike(index){
    await supabase.from('like').delete().eq('item_id', likeList[index].item_id).eq('user_id', $user.id);
...
{#if likeFlag === true}
<div class = "px-4">
  <button on:click={handleUnlikeAll} class=" bg-gray-300 p-4 rounded-full flex justify-center items-center hover:bg-gray-600">전체 삭제하기</button>
  </div>
...
<div class="p-4 border-b">
  <div class="flex justify-between items-center  gap-2">
    ...
    <button on:click = {()=>{handleUnlike(index)}} class = "text-red-600">
