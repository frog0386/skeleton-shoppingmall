<script>
	...
	const onFileSelected = async (event) => {
		...
		const { error } = await supabase
			...
			.eq('user_id', $user.id);
    user_info.profileimage = publicURL;
    console.log($profile.user_info[0].profileimage);
		$loading = false;
...
<div class="bg-white h-14 px-4 items-center border-b flex">
	...
	<div class="flex flex-1 justify-center pr-8 font-bold text-base text-gray-800">내정보관리</div>
</div>
...
{#if flag_info === true}
	<a
		...
		class="text-gray-400 h-40 flex justify-center items-center space-x-2"
	>
