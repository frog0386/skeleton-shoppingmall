<script>
	...
	import { user, profile, itemData } from '$lib/stores';
  ...
  let normalPriceSum = 0;
	onMount(async () => {
		...
		items = items;
		console.log(items);
    sailPriceSum = countSailSum();
    normalPriceSum = countNormalPriceSum();
	});

  function countSailSum() {
    let sailPriceSum = 0;
